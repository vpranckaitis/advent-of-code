...
fn solve2() -> i64 {
    ...
    loop {
        let val = test(mem.clone(), x, y);
        if val == 1 {
            if y >= 100 && test(mem.clone(), x + 99, y - 99) == 1 {
                return x * 10000 + y - 99;
            } else {
                y += 1;
            }
...
fn test(mem: Mem, x: i64, y: i64) -> i64 {
    let mut input = vec![y, x];
    let mut output = 0;
    run(&mut Program { mem, base: 0 },
        &mut || input.pop().unwrap(),
        &mut |val| output += val);
    output
}

fn run(program: &mut Program, input: &mut dyn FnMut() -> i64, output: &mut dyn FnMut(i64) -> ()) {
    let mut state = Run(0);
    loop {
        state = match state {
            Run(ip) => {
